...
export const ShippingPage = () => {
  ...
  const eanCodeFromParams = searchParams.get("eanCode");
  ...
  const [couponCodeToPay, setcouponCodeToPay] = useState('');
  ...
  const [addressSelectedFull, setAddressSelectedFull] = useState({});
  ...
  //           setcouponCodeToPay(response.code);
  ...
  const handleclick = (e, itemId, item) => {
    setAddressSelectedFull(item);
    ...
    try {
      fetch(`${process.env.REACT_APP_BACKEND_URL}/user/address/all`, {
        ...
        .then((data) => {
          ...
          if (data.length > 0) {
            setTimeout(() => {
              setAddressSelected(data[0].id);
              setAddressId(data[0].id);
              setConfrimOrder(true);
              setAddressSelectedFull(data[0]);
            }, 1000);
  ...
  useEffect(() => {
    if (addressdata.length > 0) {
      setAddressSelected(addressdata[0].id);
    }
  }, [addressdata]);
  ...
  const handlePaymentPage = () => {
    ...
    var couponCodeJson = localStorage.getItem('couponCode');
    var decodedCoupon = (couponCodeJson != null && couponCodeJson != undefined) ? JSON.parse(couponCodeJson)['code'] : '';
    const orderData = {
      // ean_code: encode,
      // quantity: quantity,
  ...
  const paymentByOnlineMethod = (respone) => {
    axios
      .post(
        ...
        {
          orderId: respone.id,
          billing_name: `${addressSelectedFull.fName} ${addressSelectedFull.lName}`,
          billing_address: addressSelectedFull.address1,
          billing_city: addressSelectedFull.city,
          billing_state: addressSelectedFull.state,
          billing_zip: addressSelectedFull.pincode,
          billing_country: addressSelectedFull.country,
          billing_tel: addressSelectedFull.phone,
          billing_email: "test@gmwil.com",
          amount: (
            (parseFloat(mrp) -
              (cartProductDetails?.product?.discount ? cartProductDetails?.product?.discount : 0) +
              (cartProductDetails?.product?.delivery_charge ?
                typeof cartProductDetails?.product?.delivery_charge === 'string' ?
                  0 :
                  parseFloat(cartProductDetails?.product?.delivery_charge) :
                0) -
              (activeCouponCode?.discount != null ?
                parseFloat(activeCouponCode?.discount) :
                couponDiscount?.discount ?
                  parseFloat(couponDiscount?.discount) :
                  0)
            ).toFixed(2)
          ),
          total: (
          ...
          currency: "INR",
          // redirect_url: window.location.origin + "/shippingConfirm",
          // cancel_url: window.location.origin + "/shippingConfirm",
          redirect_url: `${process.env.REACT_APP_BACKEND_URL}/user/payment/response`,
          cancel_url: `${process.env.REACT_APP_BACKEND_URL}/user/payment/response`,
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    ...
    cartGetAllData?.cartSlice?.initialState?.data?.cartItems?.map(
      (coupanData) => {
        if (eanCodeFromParams != undefined && eanCodeFromParams != '' && eanCodeFromParams != null) {
          if (eanCodeFromParams == coupanData.ean_code) {
            setTimeout(() => {
              // console.log("coupon ean in coupon", coupanData)
              setStatusCoupon({
                code: "",
                product: [
                  {
            ...
            // console.log(coupanData?.quantity,'coupanData?.quantitydscoupanData?.quantitycoupanData?.quantitycoupanData?.quantity')
            sum += coupanData?.quantity * coupanData?.product?.discountPrice;
            // sum =   coupanData?.product?.discountPrice*getquantity;
          }
    ...
    setMrp(sum);
    if (sum == 0 && cartGetAllData?.cartSlice?.initialState?.data?.cartItems?.length <= 0) {
      navigator("/");
    }
  }, [cartGetAllData]);
                ...
                <div className="viewOrder_PaymentPage">
                  ...
                  {/* {
                                            viewOrderStatus ?
                                                ...
                                                </a> : <p>Please wait While Confirming your Product</p>
                                        } */}
                </div>
              </div>
            </div>
                ...
                <div className="grid md:grid-cols-2 gap-5">
                    ...
                    {activeCouponCode?.discount ? null : (
                      <div className="couponInputDiv_CartPage">
                        ...
                        >
                          ...
                          <div
                            className="input_Btn_CartPage h-10"
                          >
                            <input
                              type="text"
                  ...
                  {selectPaymentMethod ? (
                    <div
                      ...
                      className="card h-fit max-w-6xl p-5 md:p-8 bg-white"
                    >
                      <div
                        style={{
                          width: "100%",
                      ...
                      >
                        <div className="rounded-xl border-2 border-solid border-black p-3 flex items-center mb-3">
                          <input
                            type="radio"
                            name="paymentMethod"
                        ...
                        <div className="rounded-xl border-2 border-solid border-black p-3 flex items-center mt-3">
                          <input
                            type="radio"
                            name="paymentMethod"
                    ...
                    <div className="carProductListMainDiv_ShippingDetails">
                      ...
                      <div className="shippingAddressMainDiv_ShippingDetails">
                        ...
                        >
                                  ...
                                  <div className="ShippingAddressMainUpper_ShippingDetails">
                                    ...
                                    >
                                      <div className="custom-checkbox">
                                        <input type="checkbox" id={item.id} className="checkbox-input"
                                          onClick={(e) => handleclick(e, item.id, item)} checked={addressSelected == item.id
                                            ? "checked"
                                            : ""} /></div>
                                      {/* <button
                                        style={{
                          ...
                          {addressdata.length > 0 ? //     width: "100%", //     border: "1px solid black", // <div style={{
                            //     padding: "1rem 1rem",
                            //     marginTop: "1rem"
                            // }}>
                          ...
                          <div className="confrimAddress_ShippingDetails">
                            <button
                              style={{
                                ...
                                marginRight: "20px",
                                fontSize: `${couponDetailHeaderFontSize * 1.1
                                  }px`,
                              }}
                                  ...
                                  {openCity ? (
                                    <ul
                                      ...
                                      style={{
                                        ...
                                        fontSize: `${productDetailsFontSize * 1.4
                                          }px`,
                                        padding: "0.5rem",
                                      }}
                                    >
                              ...
                              {address?.map((details, id) => {
                                return (
                                  <>
                                    <h2
                                      style={{
                                        fontSize: `${priceDetailsFontSize * 0.9
                                          }px`,
                                        color: "#515151",
                                        paddingLeft: "0.3rem",
                                        fontWeight: 600,
